...
# Fig 30-34. Bycatch distributions on temperature surfaces, single-panle by species ==========
  #30) Tanners; 31) opies; 32) cod; 33) yellowfin sole; 34) rock sole.)))
...
ggsave("Figures_Report/Fig30.png", plot = Tanner_map, width = 15, height = 10, dpi = 300)
...
ggsave("Figures_Report/Fig31.png", plot = Snow_map, width = 15, height = 10, dpi = 300)
...
ggsave("Figures_Report/Fig32.png", plot = cod_map, width = 15, height = 10, dpi = 300)
...
ggsave("Figures_Report/Fig33.png", plot = yellow_map, width = 15, height = 10, dpi = 300)
...
ggsave("Figures_Report/Fig34.png", plot = rock_map, width = 15, height = 10, dpi = 300)
...
# define breaks and bin lables for figure
breaks_seq <- seq(0, 225, by = 5)
labels_seq <- cut(breaks_seq, breaks = breaks_seq, include.lowest = TRUE, right = TRUE)

# clean data -- classify bait types and binning
Pots_2024_bait <- Pots_2024 %>%
  mutate(
    bait_setup = case_when(
      str_detect(NOTES, regex("Hanging Bait|\\bHB\\b", ignore_case = TRUE)) ~ "Hanging Bait",
      str_detect(NOTES, regex("Bait Bag|BB|flea bag|cod bag", ignore_case = TRUE)) ~ "Bait Bag",
      TRUE ~ NA_character_
    ),
    bag_color = case_when(
      str_detect(NOTES, regex("Black flea bag", ignore_case = TRUE)) ~ "Black",
      str_detect(NOTES, regex("Orange cod bag", ignore_case = TRUE)) ~ "Orange",
      str_detect(NOTES, regex("Bait Bag|BB", ignore_case = TRUE)) ~ "Unknown",
      TRUE ~ NA_character_
    ),
    SEX = factor(SEX, levels = c(1, 2), labels = c("Male", "Female")),
    LENGTH_bin = cut(LENGTH, breaks = seq(0, 225, 5), include.lowest = TRUE),
    LENGTH_bin = factor(LENGTH_bin, levels = levels(labels_seq)))

#Get counts 
rkc_counts <- Pots_2024_bait %>%
  filter(!is.na(bait_setup), !is.na(SEX), !is.na(LENGTH_bin)) %>%
  group_by(SEX, bait_setup, bag_color, LENGTH_bin) %>%
  summarise(n = n(), .groups = "drop") 

# Apply consistent binning
Pots_2024_bait <- Pots_2024_bait %>%
  mutate(
    LENGTH_bin = cut(LENGTH, breaks = breaks_seq, include.lowest = TRUE, right = TRUE),
    LENGTH_bin = factor(LENGTH_bin, levels = levels(labels_seq)) # Ensure 0–5 is kept
  )


#Plot by sex, by Bait Type for size-comps 
ggplot(rkc_counts, aes(x = LENGTH_bin, y = n, fill = bag_color)) +
  geom_col(position = "dodge", 
           aes(fill = ifelse(bait_setup == "Bait Bag", bag_color, "Hanging")),
           color = "black", 
           linewidth = 0.2) +
  facet_grid(rows = vars(SEX), cols = vars(bait_setup)) +
  scale_fill_manual(
    values = c("Black" = "#444444", "Orange" = "#E69F00", "Unknown" = "grey80", "Hanging" = "lightblue"),
    na.translate = FALSE,
    name = "Bait Container Type"
  ) +
  labs(
    title = "RKC Length-Frequency by Sex and Bait Type",
    x = "Carapace Width (mm)",
    y = "Count"
  ) +
  scale_x_discrete(
    breaks = levels(labels_seq),
    labels = function(x) {
      # only label breaks that are multiples of 50
      start_vals <- as.numeric(gsub("\\((.+),.*", "\\1", x))
      ifelse(start_vals %% 50 == 0, paste0(start_vals), "")
    }
  )+
  theme_bw() +
  theme(
...
#ggsave(plot = Fig35, "./Figures_Report/Fig35.png", height=7, width=10, units="in")
...
Pots_2024_bait <- Pots_2024_bait %>%
  mutate(
    bait_facet = factor(
      case_when(
        bait_setup == "Hanging Bait" ~ "Hanging Bait",
        bait_setup == "Bait Bag" & bag_color == "Black" ~ "Bait Bag (Black)",
        bait_setup == "Bait Bag" & bag_color == "Orange" ~ "Bait Bag (Orange)",
        bait_setup == "Bait Bag" & bag_color == "Unknown" ~ "Bait Bag (Unknown)",
        bait_setup == "Bait Bag" ~ "Bait Bag (All)",
        TRUE ~ NA_character_
      ),
      levels = c("Hanging Bait", "Bait Bag (All)", "Bait Bag (Black)", "Bait Bag (Orange)", "Bait Bag (Unknown)")
    ),
    plot_color = case_when(
      bait_facet == "Hanging Bait" ~ "Hanging",
      bait_facet == "Bait Bag (Black)" ~ "Black",
      bait_facet == "Bait Bag (Orange)" ~ "Orange",
      bait_facet == "Bait Bag (Unknown)" ~ "Unknown",
      bait_facet == "Bait Bag (All)" ~ "All",
      TRUE ~ NA_character_
    )
  )

# Recount with "All Bait Bags"
rkc_counts <- Pots_2024_bait %>%
  filter(!is.na(bait_facet), !is.na(SEX), !is.na(LENGTH_bin)) %>%
  group_by(SEX, bait_facet, plot_color, LENGTH_bin) %>%
  summarise(n = n(), .groups = "drop")

# Base plot
ggplot(rkc_counts, aes(x = LENGTH_bin, y = n, fill = plot_color)) +
  geom_col(color = "black", width = 0.9) +
  facet_grid(rows = vars(bait_facet), cols = vars(SEX)) +
  scale_fill_manual(
    values = c(
      "Black" = "#444444",
      "Orange" = "#E69F00",
  ...
  scale_x_discrete(
    breaks = levels(Pots_2024_bait$LENGTH_bin),
    labels = function(x) {
      starts <- as.numeric(gsub("\\((.+),.*", "\\1", x))
      ifelse(starts %% 50 == 0, as.character(starts), "")
    }
  ) +
  labs(
    title = "RKC Length-Frequency by Sex and Bait Type",
...
ggsave(plot = Fig35_alt, "./Figures_Report/Fig35_alt.png", height=7, width=10, units="in")
